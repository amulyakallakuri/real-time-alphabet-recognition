...
CNN = Model('ResNet')
CNN.load_classifier(path = r'trained_networks/resnet50_10epo')


def draw_region(image, box):
    global id
    id+=1
    # cropped_image = cv2.rectangle(image, (center[0] - 130, center[1] - 130),
    #     (center[0] + 130, center[1] + 130), (0, 0, 255), 2)
    cropped_image = cv2.rectangle(image, (box[0],box[1]),
        (box[2],box[3]), (0, 0, 255), 2)
    hand_image = cropped_image[box[0]:box[1],box[2]:box[3]]
    hand_image = cv2.cvtColor(hand_image, cv2.COLOR_BGR2RGB)
    cv2.imwrite('collected_pics/'+'B_'+str(id)+'.jpg',hand_image)
    return hand_image, cropped_image
...
def get_char(gesture):
    ...
    label, out = CNN.predict(classes, gesture)
    return label, out
...
def frame_video_stream(names, curr_char, prev_char, word, sentence, *args):
    ...
    if results.multi_hand_landmarks:
        ...
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            left = int(max(min(x)-0.01,0)* image_width)
            right = int(min(max(x)+0.01,1)* image_width)-1
            upper = int(max(min(y)-0.01,0)* image_height)
            down = int(min(max(y)+0.01,1)* image_height)-1
            box = [left,right,upper,down]
            
            center = np.array([np.mean(x) * image_width, np.mean(y) * image_height]).astype('int32')
            max = np.array([np.max(x) * image_width, np.max(y) * image_height]).astype('int32')
            min = np.array([np.min(x) * image_width, np.min(y) * image_height]).astype('int32')
            #image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

            cropped_img, full_img = draw_region(image, box)#left,right,upper,down,
